...
import { ArrowLeft, Save, X, Plus, Minus, Folder, Trash2 } from 'lucide-react'
import Link from 'next/link'
import { useRouter, useParams } from 'next/navigation'

const BASE_URL = process.env.NEXT_PUBLIC_BASE_URL

export default function EditSourceCode() {
  const router = useRouter()
  const params = useParams()
  const sourceCodeId = params.id
  ...
  const fetchSourceCode = async () => {
    try {
      const response = await fetch(`${BASE_URL}/edit/source-code/${sourceCodeId}`)
      ...
      if (data.success) {
        ...
        setFormData({
          ...sc,
          download_available: Boolean(sc.download_available)
        })

        // Parse technologies and features if they're JSON strings
        if (sc.technologies) {
          const techArray = typeof sc.technologies === 'string' 
            ? JSON.parse(sc.technologies) 
            : sc.technologies
          setTechnologies(techArray.length > 0 ? techArray : [''])
        }

        if (sc.features) {
          const featArray = typeof sc.features === 'string'
            ? JSON.parse(sc.features)
            : sc.features
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(`❌ ${data.error || 'Error updating source code'}`)
      }
    } catch (error) {
      console.error('Error:', error)
      alert('❌ Error updating source code')
    } finally {
      setLoading(false)
    }
  }
  ...
  if (fetching) {
    return (
      <div className="admin-dashboard">
        <div className="admin-loading">Loading source code...</div>
      </div>
    )
  }
      ...
      >
        <div className="admin-form-header">
          ...
          <div className="admin-form-title-section">
            <h1 className="admin-form-title">Edit Source Code</h1>
            <p className="admin-form-subtitle">Update source code information</p>
          </div>
        </div>
